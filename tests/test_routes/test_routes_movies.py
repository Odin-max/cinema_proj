...
async def prepare_users(session: AsyncSession):
    ...
    u1 = User(
        ...
        hashed_password="pwdhash1",
        group_id=default_group.id
    )
    u2 = User(
        email="user2@example.com",
        hashed_password="pwdhash2",
        group_id=default_group.id
    )
...
@pytest.mark.anyio
async def test_list_movies_filters_and_pagination(client: AsyncClient, session: AsyncSession):
    cert = CertificationModel(name="PG")
    session.add(cert)
    await session.commit()

    m1 = MovieModel(
        name="Alpha", year=2000, time=100, imdb=7.0, votes=10,
        meta_score=50, gross=100000, description="first movie", price=5.0,
        certification_id=cert.id
    )
    m2 = MovieModel(
        name="Beta", year=2001, time=110, imdb=8.0, votes=20,
        meta_score=60, gross=200000, description="second movie", price=6.0,
        certification_id=cert.id
    )
    m3 = MovieModel(
        name="Gamma", year=2000, time=120, imdb=9.0, votes=30,
        meta_score=70, gross=300000, description="third entry", price=7.0,
        certification_id=cert.id
    )
...
async def test_favorite_endpoints(client: AsyncClient, session: AsyncSession):
    ...
    movie = MovieModel(name="FavMe", year=2010, time=90, imdb=5.5, votes=5,
                       meta_score=40, gross=50000, description="", price=3.0,
                       certification_id=cert.id)
    session.add(movie)
    await session.commit()
    ...
    fav = (await session.execute(
        select(FavoriteModel).where(FavoriteModel.user_id == 1)
    )).scalar_one()
...
@pytest.mark.anyio
async def test_movie_detail_likes_dislikes_and_rating(client: AsyncClient, session: AsyncSession):
    cert = CertificationModel(name="PG")
    ...
    movie = MovieModel(name="DetailMe", year=2020, time=100, imdb=7.5, votes=50,
                       meta_score=75, gross=150000, description="desc", price=4.0,
                       certification_id=cert.id)
...
async def test_like_movie_endpoint(client: AsyncClient, session: AsyncSession):
    ...
    movie = MovieModel(name="LikeMe", year=2022, time=95, imdb=7.0, votes=2,
                       meta_score=70, gross=100000, description="", price=2.5,
                       certification_id=cert.id)
    session.add(movie)
    await session.commit()
    ...
    like = (await session.execute(
        select(MovieLikeModel).where(MovieLikeModel.user_id == 1)
    )).scalar_one()
...
async def test_comments_and_replies(client: AsyncClient, session: AsyncSession):
    ...
    movie = MovieModel(name="CommentMe", year=2015, time=80, imdb=6.0, votes=5,
                       meta_score=60, gross=80000, description="", price=1.0,
                       certification_id=cert.id)
    session.add(movie)
    await session.commit()

    r = await client.post(f"/movies/{movie.id}/comments", json={"text": "Hello", "parent_id": None})      
    assert r.status_code == status.HTTP_201_CREATED
    root = r.json()
    assert root['text'] == 'Hello' and root['replies'] == []

    r = await client.post(f"/movies/{movie.id}/comments", json={"text": "Reply", "parent_id": root['id']})
    assert r.status_code == status.HTTP_201_CREATED

    r = await client.get(f"/movies/{movie.id}/comments")
    assert len(r.json()) == 1

    r = await client.post(f"/movies/{movie.id}/comments", json={"text": "Bad", "parent_id": 999})
    assert r.status_code == status.HTTP_400_BAD_REQUEST

@pytest.mark.anyio
async def test_rate_and_purchase_and_delete_movie(client: AsyncClient, session: AsyncSession):
    cert = CertificationModel(name="PG")
    session.add(cert)
    await session.commit()

    movie = MovieModel(
        name="PurchaseMe", year=2018, time=85, imdb=7.2, votes=10,
        meta_score=65, gross=120000, description="", price=2.0,
        certification_id=cert.id
    )
    session.add(movie)
    await session.commit()
