...
from app.models.movie_models import CertificationModel, CertificationCreate, CertificationUpdate, CertificationRead
...
async def update_certification(
    ...
    existing = await db.scalar(
        select(CertificationModel).where(
            CertificationModel.name == cert_in.name,
            CertificationModel.id != cert_id
        )
    )
    if existing:
        raise HTTPException(
