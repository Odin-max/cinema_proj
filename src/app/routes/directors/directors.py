...
from app.models.movie_models import DirectorModel, DirectorCreate, DirectorRead, DirectorUpdate
...
async def update_director(
    ...
    existing = await db.scalar(
        select(DirectorModel).where(
            DirectorModel.name == director_in.name,
...
async def delete_director(
    ...
    await db.execute(
        sql_delete(DirectorModel).where(DirectorModel.id == director_id)
    )
    await db.commit()
    return None
