...
async def create_star(
    ...
    existing = await db.scalar(
        select(StarModel).where(StarModel.name == star_in.name)
    )
    if existing:
        raise HTTPException(
...
async def update_star(
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Star name already in use"
        )
...
async def delete_star(
    ...
    await db.execute(
        sql_delete(StarModel).where(StarModel.id == star_id)
    )
    await db.commit()
    return None